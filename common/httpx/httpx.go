...
func New(options *Options) (*HTTPX, error) {
	...
	if httpx.Options.SniName != "" {
		tlsClientConfig.ServerName = httpx.Options.SniName
	}
	if httpx.Options.Protocol == HTTP11 {
		// Force HTTP/1.1 only: ALPN must not negotiate "h2" so the server cannot use HTTP/2.
	...
	transport := &http.Transport{
		...
		DialTLSContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
			if options.TlsImpersonate {
				...
				if httpx.Options.SniName != "" {
					cfg.ServerName = httpx.Options.SniName
				}
				return httpx.Dialer.DialTLSWithConfigImpersonate(ctx, network, addr, cfg, impersonate.Random, nil)
			}
			if httpx.Options.Protocol == HTTP11 {
				// Use DialTLSWithConfig so the TLS handshake advertises only http/1.1 via ALPN.
	...
	if httpx.Options.Protocol == HTTP11 {
		// Disable HTTP/2 upgrade: no next-proto handler so Transport stays on HTTP/1.1.
